...
class NewTermView(LoginRequiredMixin, View):
    ...
    def _get_context(self, client, term, summary, cfg, form=None):
        ...
        return {
            ...
            "summary": summary,
            "remaining": remaining,
            "conv_dev_hours": compute_converted_dev_hours(remaining, cfg),
            "mig_sup_hours": compute_migrated_support_hours(remaining, cfg),
            "max_migrate": cfg.max_migrate_hours,
            "form": form
            or NewTermForm(initial={"monthly_hours": term.monthly_hours}),
            "fmt_hours": fmt_hours,
        }
    ...
    def post(self, request, pk):
        ...
        dev_hours_from_conversion = 0.0
        migrated_support_hours = 0.0

        if carry_type == ClientTerm.CARRY_CONVERT_DEV:
            dev_hours_from_conversion = compute_converted_dev_hours(
                remaining, cfg
            )
        elif carry_type == ClientTerm.CARRY_MIGRATE:
            migrated_support_hours = compute_migrated_support_hours(
                remaining, cfg
            )
